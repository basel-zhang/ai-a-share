...
def parse_final_state_response(response):
    if isinstance(response, str):
        try:
            ...
            _log.exception("Error parsing response:")
            return None
    return response
...
def create_workflow():
    ...
    # Start node
    workflow.add_node(a_share_data_agent.__name__, a_share_data_agent)
    workflow.set_entry_point(a_share_data_agent.__name__)
    ...
    for analyst_key in selected_analysts:
        ...
        workflow.add_edge(a_share_data_agent.__name__, node_name)
    # Always add risk and portfolio management
    workflow.add_node(risk_management_agent.__name__, risk_management_agent)
    workflow.add_node(portfolio_management_agent.__name__, portfolio_management_agent)
    # Connect selected analysts to risk management
    for analyst_key in selected_analysts:
        node_name = analyst_nodes[analyst_key][0]
        workflow.add_edge(node_name, risk_management_agent.__name__)
    workflow.add_edge(risk_management_agent.__name__, portfolio_management_agent.__name__)
    workflow.add_edge(portfolio_management_agent.__name__, END)
    return workflow
